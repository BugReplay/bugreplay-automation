...
function actions(dispatch) {
    return {
        ...
        recordingCreated: cancel => dispatch({
...
function api(window, settings) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (error.error_name === 'ErrUserNotLoggedIn' ||
                    (error.error_name === 'api_error' &&
                        error.message === 'Invalid or expired User Session. Login required')) {
    ...
    function postAjax(path, data, opts) {
        return wrappedAjax(Object.assign({ url: urlFor(path), method: 'POST', data }, opts));
    }
    ...
    function uploadVideo(videoBlob) {
        return __awaiter(this, void 0, void 0, function* () {
            // failed to GetVideosLeftForBillingCycle: sql: no rows in result set
            try {
                return yield uploadAsset('rawVideo', videoBlob);
            }
            catch (err) {
                if (err.message.includes('GetVideosLeftForBillingCycle')) {
                    Object.assign(err, { errorMessageForUser: 'You are out of videos.' });
                }
                throw err;
            }
        });
    ...
    function sourceMapStorageCheck(sourceMapHash) {
        ...
        return wrappedAjax({
            url: urlFor('/api/asset/source_map'),
            method: 'GET',
    ...
    function uploadRecordingAssets(recorders, fullstack) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const uploadingUrlData = urlData
                ? uploadUrlLogs(urlData)
                : Promise.resolve({ jwt: null });
...
if (!browser_detection_1.capabilities.isFirefox) {
    navigator.permissions.query({ name: 'microphone' }).then(permissionStatus => {
        store.dispatch({
            type: 'MICROPHONE_PERMISSION_CHANGED',
            payload: { state: permissionStatus.state },
        });
...
function checkTrello(integrations, api, dispatchBackgroundActions) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            console.log(error);
            // BR-936 Trello will trigger an error on boards if user has multiple accounts logged
            // and one of those has the trello integration but the current one does not (trello calls will still go through but boards will fail).
            // As it's not possible to discriminate the user on the server it will simply throw an error.
            // As this is likely to only occur in that case I think the easiest/best path is to silence this error on the extension as it would only be
            // misleading to the end user. We'll still log it in the console though.
...
function startRecording(browser, baseUrl, apiBaseUrl, tab, type, navigator, screen, tabCapture, desktopCapture, MediaRecorder, Blob, chrome, capabilities, dispatchBackgroundActions, recordMic, countdownTimer, uuid, recordVideo) {
    ...
    try {
        tabRecorder = tab_1.createTabRecorder(browser, tab.id, chrome, capabilities, dispatchBackgroundActions.debuggerDetached, dispatchBackgroundActions.onNetworkError, error => {
            if (error)
    ...
    dispatchBackgroundActions.recordingCreated(cancel);
    function stop() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
...
function createFirefoxNetworkRecorder(browser, tabId, uuid) {
    ...
    const log = (type, data) => {
        if (started) {
            buffer += JSON.stringify({ type, data });
        }
    };
    ...
    function webrequestToRequestWillBeSent(info) {
        ...
        return {
            ...
            frameId: String(info.frameId),
            loaderId: '',
            wallTime: info.timeStamp / 1000,
            initiator: {
                type: 'other',
            },
            request: req,
    ...
    function onSendHeaders(info) {
        ...
        if (!request)
            return;
        const headers = {};
        for (let i = 0, l = info.requestHeaders.length; i < l; i++) {
            const h = info.requestHeaders[i];
            headers[h.name] = h.value;
        }
        request.headers = headers;
        const requestData = webrequestToRequestWillBeSent(request);
        log('requestWillBeSent', requestData);
        requests.set(info.requestId, requestData);
    }
    function webrequestToResponseReceived(info) {
        const mimeType = info.headers ? info.headers['content-type'] : '';
        const resp = {
            status: info.statusCode,
            statusText: info.statusLine,
            headers: info.headers,
            url: info.url,
            mimeType,
    ...
    function onHeadersReceived(info) {
        const headers = {};
        for (let i = 0, l = info.responseHeaders.length; i < l; i++) {
            const h = info.responseHeaders[i];
            headers[h.name] = h.value;
        }
        Object.assign(info, { headers });
        log('responseReceived', webrequestToResponseReceived(info));
    }
    function webrequestToLoadingFinished(info) {
        return {
            timestamp: info.timeStamp / 1000,
            requestId: info.requestId,
            encodedDataLength: 0,
...
const urlLog = []; // tslint:disable-line:readonly-array
...
function enableDomain(tabId, chrome, domain, cb) {
    chrome.debugger.sendCommand({ tabId }, domain + '.enable', opts[domain], () => {
        cb(chrome.runtime.lastError);
    });
}
...
function createChromeTabRecorder(tabId, chrome, capabilities, onDebuggerDetached, onNetworkError, onInitialized, uuid) {
    ...
    chrome.tabs.onUpdated.addListener(urlEventListener);
    function start() {
        started = true;
    ...
    function stop() {
        ...
        chrome.tabs.onUpdated.removeListener(urlEventListener);
        return {
    ...
    const debuggerAndNetworkDomains = ['Debugger', 'Network'];
    ...
    attachDebugger(tabId, chrome, debuggerVersion, err => {
        ...
        enableDomains(tabId, chrome, debuggerAndNetworkDomains.concat(consoleDomains), () => {
            initialized = true;
            if (started)
                onInitialized();
        });
...
function createFirefoxTabRecorder(browser, tabId, onInitialized, uuid) {
    ...
    function start() {
        ...
        browser.tabs.onUpdated.addListener(urlEventListener);
        onInitialized();
    }
    function stop() {
        browser.tabs.onUpdated.removeListener(urlEventListener);
        return {
            network: networkRecorder.stop(),
            console: {
                data: '',
...
function base64EncodedToDecodedBlob(Blob, str, contentType) {
    const raw = atob(str);
    ...
    const stop = lodash_1.once(() => {
        ...
        if (recordingTabId) {
            browser.tabs.sendMessage(recordingTabId, {
                type: 'FIREFOX_VIDEO_RECORDING_STOP',
            });
        }
        return stopped;
    });
    const teardown = lodash_1.once(() => {
        browser.runtime.onMessage.removeListener(listener);
        stop();
        if (recordingTabId) {
            browser.tabs.remove(recordingTabId);
        }
    });
    const teardownWithoutClose = lodash_1.once(() => {
        browser.runtime.onMessage.removeListener(listener);
        stop();
        if (recordingTabId) {
            browser.tabs.update(recordingTabId, {
                active: true,
            });
        }
    });
    const listener = (message) => {
        try {
            switch (message.type) {
                ...
                case 'FIREFOX_VIDEO_RECORDING_COUNTDOWN_START': {
                    browser.windows.update(tab.windowId, {
                        focused: true,
                    });
                    browser.tabs.update(tab.id, {
                ...
                case 'FIREFOX_VIDEO_RECORDING_SUCCESS': {
                    teardown();
                    return resolveStopped({
                        frameTimes,
                        blob: new Blob(frames),
                    });
                }
                case 'FIREFOX_VIDEO_RECORDING_FAILURE': {
                    teardownWithoutClose();
                    return rejectStopped({ message: message.error });
                }
                case 'FIREFOX_VIDEO_RECORDING_NOT_ALLOWED': {
                    teardownWithoutClose();
                    // return onCancel()
                }
                case 'FIREFOX_VIDEO_RECORDING_NOT_ALLOWED_PREVIOUSLY': {
                    teardownWithoutClose();
                    // return onCancel()
                }
            }
    ...
    try {
        ...
        browser.tabs
            ...
            if (!createdWindow) {
                teardown();
                return onInitialized({
                    message: `Popup to stop recording failed to initialize. Please enable popups for ${baseUrl}`,
                });
            }
            recordingTabId = createdWindow.id;
        })
            .catch(error => {
            console.error(error);
            stop();
...
function logErrorAndReturnCopyForUser(settings, api, initialState, action) {
    ...
    if ('errorHtmlForUser' in error) {
        return { __html: error.errorHtmlForUser };
    }
...
function reducer(settings, api, initialState = emptyState, action) {
    ...
    switch (action.type) {
        ...
        case 'MICROPHONE_PERMISSION_CHANGED': {
            return Object.assign(Object.assign({}, initialState), { recordMic: initialState.recordMic && action.payload.state == 'granted' });
        }
        ...
        case 'DEBUGGER_DETACHED': {
            ...
            if (!initialState.recording.started && !initialState.countdownTimer) {
        ...
        case 'REPORT_PROCESSING_FAILURE': {
            ...
            return Object.assign(Object.assign({}, initialState), { reports: Object.assign(Object.assign({}, initialState.reports), { processing, failed: [lodash_1.omit(failed, 'uploadingAssets')].concat(
                    // omit the uploadingAssets here as we want those requests to be retried
                    initialState.reports.failed) }) });
        }
        case 'RECORDING_START_SUCCESS': {
            if (initialState.recording.stoppedByUser && initialState.countdownTimer) {
        ...
        case 'RECORDING_CREATED': {
            return Object.assign(Object.assign({}, initialState), { recording: Object.assign(Object.assign({}, initialState.recording), { cancel: action.payload.cancel }) });
        }
...
Object.defineProperty(exports, "__esModule", { value: true });
const browser_detection_1 = require("../browser-detection");
const { isOpera } = browser_detection_1.capabilities;
...
function startTimer(browserAction) {
    ...
    if (!isOpera) {
        browserAction.setBadgeBackgroundColor({ color: [0, 0, 0, 255] });
        setBadgeText('0:00');
    }
    else {
        browserAction.setBadgeBackgroundColor({ color: [255, 0, 0, 255] });
        setBadgeText('●');
    }
...
function createTimer(browserAction) {
    let timer = null;
    function start() {
        if (timer)
            timer.stop();
        timer = startTimer(browserAction);
    }
    function stop() {
        if (timer) {
            timer.stop();
...
function chromeMajorVersion(navigator) {
    return majorVersion('Chrome', navigator);
}
function firefoxMajorVersion(navigator) {
    return majorVersion('Firefox', navigator);
}
function operaMajorVersion(navigator) {
    return majorVersion('OPR', navigator);
}
function detectCapabilities(browser, navigator) {
    const chromeVersion = chromeMajorVersion(navigator);
    const firefoxVersion = firefoxMajorVersion(navigator);
    const operaVersion = operaMajorVersion(navigator);
    const isChrome = chromeVersion != null;
    const isFirefox = firefoxVersion != null;
    const isOpera = operaVersion != null;
    ...
    const disableTabCapture = isFirefox || chromeVersion === 57;
    ...
    const autoplayVideoPreview = isFirefox && firefoxVersion >= 65 && firefoxVersion <= 70;
    const canRecordMic = true;
    return {
        isFirefox,
        isChrome,
        isOpera,

...
$(() => {
    const uniqueid = 'br-screen-recorder-extension';
    let drag, dragx, dragy;
    let dragging = false;
    let dragged = false;
    let mousedown = false;
    let cameraon = true;
    ...
    function injectCode(inject) {
        if (inject) {
            ...
            $('body').append(wrapper);
            // Inject the iframe
            let iframeinject = "<div id='detect-iframe'><div id='hide-camera' class='camera-hidden'><img src='" +
                chrome.extension.getURL('/img/camera/close.svg') +
                "' class='noselect'></div><div id='mic' title='Enable/disable microphone'><img src='" +
                chrome.extension.getURL('/img/camera/mic-off.svg') +
                "'/></div></div><div id='wrap-iframe' class='notransition'><iframe src='" +
                chrome.extension.getURL('./html/camera.html') +
                "' allow='camera'></iframe></div>";
            ...
            // remove the camera iframe from the DOM
            $('#' + uniqueid).remove();
        }
    }
    // Switch system/microphone audio on and off
    function audioEnable(_type, enable) {
    ...
    function micEnabled(enable) {
        micon = enable;
        if (enable) {
            $('#' + uniqueid + ' #mic img').attr('src', chrome.extension.getURL('/img/camera/mic-off.svg'));
            audioEnable('mic', true);
        }
        else {
            $('#' + uniqueid + ' #mic img').attr('src', chrome.extension.getURL('/img/camera/mic.svg'));
    ...
    function mouseMove(e) {
        ...
        if (dragging && cameraon) {
            ...
            $('#' + uniqueid + ' #detect-iframe').css('left', 
            // @ts-ignore
            e.clientX - dragx - $(window).scrollLeft() + 'px');
            $('#' + uniqueid + ' #detect-iframe').css('top', 
            // @ts-ignore
    ...
    // Hide/show camera
    $(document).on('click', '#' + uniqueid + ' #hide-camera', () => {
        cameraEnabled(false);
    });
    ...
    $(document).on('click', '#' + uniqueid + ' #mic', () => {
        micEnabled(!micon);
    });
    // Show camera settings
    $(document).on('mouseover', '#' + uniqueid + ' #detect-iframe', e => {
        if (cameraon) {
            $('.camera-hidden').removeClass('camera-hidden');
        }
    });
    // Hide camera settings
    $(document).on('mouseout', '#' + uniqueid + ' #detect-iframe', e => {
        $('#' + uniqueid + ' #hide-camera').addClass('camera-hidden');
        // $('#' + uniqueid + ' #mic').addClass('camera-hidden')
    });
    // Detect a click on the camera container (possible drag)
    $(document).on('mousedown', '#' + uniqueid + ' #detect-iframe', e => {
        if (e.which !== 1)
            return;
        e.stopPropagation();
        drag = $('#' + uniqueid + ' #wrap-iframe');
